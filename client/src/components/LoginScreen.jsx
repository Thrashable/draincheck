    ...
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      ...
      >
        <div className="bg-surface-raised border border-surface-border rounded-2xl p-10 text-center relative overflow-hidden">
          ...
          {/* Explore with fictional sample data — no login, no backend required */}
          <button
            onClick={onDemo}
            className="mt-3 inline-flex items-center justify-center gap-2 w-full py-3 px-6 bg-transparent border border-surface-border text-text-secondary font-body font-medium text-sm rounded-xl transition-all duration-200 hover:border-accent-coral/40 hover:text-text-primary active:scale-[0.99]"
          >
            Explore the live demo <span className="text-accent-coral">&rarr;</span>
          </button>

          <p className="mt-6 text-text-muted text-xs font-body">
            Sign-in requests <span className="text-text-secondary">read-only</span> Gmail access. Email contents are never stored or shared.
          </p>
          <p className="mt-2 text-text-muted text-xs font-body">
            Just looking? The demo runs on fictional sample data.
          </p>
        </div>
