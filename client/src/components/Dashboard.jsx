...
import { DEMO_SUBSCRIPTIONS, DEMO_LAST_SCANNED } from '../data/demoSubscriptions';

export default function Dashboard({ user, demo = false, onLogout }) {
  ...
  useEffect(() => {
    if (demo) {
      // Demo mode: render fictional sample data, no network calls.
      setSubscriptions(DEMO_SUBSCRIPTIONS);
      setLastScanned(DEMO_LAST_SCANNED);
      setLoaded(true);
      return;
    }
  ...
  async function handleScan() {
    ...
    setError(null);
    if (demo) {
      // Simulate a scan so recruiters see the progress animation + result.
      ...
      <header className="border-b border-surface-border bg-surface-base/80 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {demo && (
              <span className="px-2 py-0.5 rounded-full text-[10px] font-body font-semibold bg-accent-coral/15 text-accent-coral border border-accent-coral/20 uppercase tracking-wide">Demo data</span>
            )}
          </div>
